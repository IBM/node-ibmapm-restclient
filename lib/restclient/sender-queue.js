...
function sendData(options, sendmethod, task, quequeName, payloadString) {
    ...
    var req = sendmethod.request(options, function(res) {
        if (res.statusCode >= 200 && res.statusCode < 300 || res.statusCode === 409) {
            ...
            ' Response statusCode: ', res.statusCode);
            if (task.retry < MAX_RETRY_TIMES && !task.GET && task.type !== 'metrics: ') {
                task.retry++;
                var i = task.retry;
                while (i >= 0) {
    ...
    req.on('error', function(error) {
        if (task.retry < MAX_RETRY_TIMES && !task.GET &&
            (['metrics: ', 'dc:', 'metadata:', 'amui:'].indexOf(task.type) < 0
            || task.type.indexOf('resources:') < 0)) {
            task.retry++;
